...
3. 在本次中文样本上，已实际验证模型中 `DeepSeek V3.2` 使用最少 token（256，`1.49 chars/token`），其次是 `Seed OSS`（260）和 `GLM-4.5`（263）。
...
| 2 | ByteDance Seed OSS | 260 | 1.47 |
| 3 | Z.ai GLM-4.5 | 263 | 1.45 |
| 4 | Qwen3 8B | 268 | 1.42 |
| 5 | Baidu ERNIE 4.5 | 273 | 1.40 |
| 6 | Cohere Command A+ | 282 | 1.35 |
| 7 | xAI Grok-2 | 291 | 1.31 |
| 8 | OpenAI GPT-4o | 324 | 1.18 |
| 9 | Mistral Small 3.1 | 376 | 1.01 |

`DeepSeek V3.2` 相比 `Mistral Small 3.1` 在该中文文本上少使用 120 tokens，减少约 31.9%。这说明对中文长上下文、中文 RAG 文档或中文提示词成本评估时，不能直接复用英文场景下的 tokenizer 印象。
...
| 7 | Mistral Small 3.1 | 229 | 4.76 |
| 8 | ByteDance Seed OSS | 231 | 4.71 |
| 9 | Baidu ERNIE 4.5 | 245 | 4.44 |
...
DeepSeek V3.2 在当前 `transformers` 环境下通过完整 `AutoTokenizer` 初始化会触碰无关的模型配置兼容问题；示例直接读取官方 `tokenizer.json`，只保留本任务所需的精确纯文本分词行为。
...
bpe-demo --models openai,qwen,grok,deepseek,glm,seed,mistral,ernie,cohere \
